...
class Segment extends Widget_Base {
	
	public function __construct($data = [], $args = null) {
      parent::__construct($data, $args);

      wp_register_script( 'pnwt-segment-handle', POWERNODE_ELEXTS_URL . 'assets/js/segment.js', [ 'elementor-frontend' ], POWERNODEWT_CORE_VERSION, true );
   	}
	
	public function get_name() {
		return 'powernode-segment';
	}
	
	public function get_title() {
		return __( 'Segment', 'powernodewt-core' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'view_type',
			[
				'label' => esc_html__( 'Segment View', 'powernodewt-core' ),
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::SELECT,
				'default' => '',
				'options' => powernodewt_categories( 'segment-cat', 'id' ),
			]
		);
		...
		$this->start_controls_section(
			...
			[
				'label' => esc_html__( 'Segment Setttings', 'powernodewt-core' ),
			]
		);
		
		$this->add_control(
			'segment_loop_post_thumbnail',
			[
		...
		$this->add_group_control(
			...
			[
				'name' => 'segment_loop_post_image', // Usage: `{name}_size` and `{name}_custom_dimension`, in this case `thumbnail_size` and `thumbnail_custom_dimension`.
				'default' => 'large',
				'separator' => 'none',
				'condition' => [
					'segment_loop_post_thumbnail' => '1',
				],
			]
		);
		
		$this->add_control(
			'segment_loop_post_fancy_date',
			[
		...
		$this->add_control(
			'segment_loop_post_excerpt_length',
			[
		...
		$this->add_control(
			'segment_loop_post_remove_items_padding',
			[
	...
	protected function render() {
		...
		echo powernodewt_segment( $settings );
		
	}
	
	public function get_script_depends() {
		if( is_user_logged_in() ) return [ 'pnwt-global', 'pnwt-segment-handle' ];
		return [];
	}
}