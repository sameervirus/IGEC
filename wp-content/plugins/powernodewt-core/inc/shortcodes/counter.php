...
*/
if ( ! function_exists( 'powernodewt_counter' ) ) :

	function powernodewt_counter( $atts = array(), $content = null ) {
		...
		$css_output							= '';
		$args								= $atts;
		$args['id']  						= powernodewt_uniqid('powernode-counter-');
		...
		$args['wrap_atts']['class'][] 		= 'powernode-counter';
		...
		$data_attr['class'] = array( 'pnwt-counter', 'pnwt-bxim', 'fbox-2', 'mb-40' );
		...
		$args['sec_content'] .= '<div class="fbox-txt">';
		...
		if( !empty( $counter_number ) ) {
			...
			$counter_num_attr['class'][] = ( !in_array( $counter_num_font_size, array( '', 'custom' ) ) ) ?  $counter_num_size . '-' . $counter_num_font_size : '';
			$counter_num_attr['class'][] = ( !in_array( $counter_num_font_weight, array( '', 'medium' ) ) ) ? 'fw-' . $counter_num_font_weight : '';
			$counter_num_attr['class'][] = ( !in_array( $counter_num_font_style, array( '', 'normal', 'default' ) ) ) ? 'font-' . $counter_num_font_style : '';
			...
			$counter_num_attr['class'][] = ( !in_array( $counter_num_color, array( '', 'custom', 'default' ) ) ) ? $counter_num_color . '-color' : '';
			$counter_num_attr['class'][] = ( ! empty( $counter_num_el_classes ) ) ? $counter_num_el_classes : '';
			
			if( in_array( $counter_num_size, array( '', 'h3' ) ) ) {
				$args['sec_content'] .= '<h3 ' . powernodewt_stringify_atts( $counter_num_attr ) . '>' . $counter_number_prefix . '<span class="count-element">' . do_shortcode( $counter_number ) . '</span>' . $counter_number_sufix . '</h3>';
			} else {
				$args['sec_content'] .= '<'. $counter_num_size .' ' . powernodewt_stringify_atts( $counter_num_attr ) . '>' . $counter_number_prefix . '<span class="count-element">' . do_shortcode( $counter_number ) . '</span>' . $counter_number_sufix . '</'. $counter_num_size .'>';
			}
			
			$counter_num_css_ar = array();
			if( $counter_num_color == 'custom' && !empty( $counter_num_color_custom ) ) $title_css_ar['color'] = $counter_num_color_custom;
			if( $counter_num_font_size == 'custom' && !empty( $counter_num_custom_font_size ) ) $counter_num_css_ar['font-size'] = $counter_num_custom_font_size;
			if( $counter_num_line_height != '' ) $counter_num_css_ar['line-height'] = $counter_num_line_height;
			if( $counter_num_letter_spacing != '' ) $counter_num_css_ar['letter-spacing'] = $counter_num_letter_spacing;
			$css_output .= powernodewt_output_css( array( '#'. $args['id'] . ' .bx-counter' => $counter_num_css_ar ) );
		}
		...
		$html = '';
		ob_start();
		powernodewt_exts_get_template( 'shortcodes/counter', $args );
		$html = ob_get_clean();
...
add_shortcode( 'powernode_counter', 'powernodewt_counter' );