...
if ( ! function_exists( 'powernodewt_blockquote' ) ) :

	function powernodewt_blockquote( $atts, $content = null  ) {
		...
		$css_output							= '';
		$args								= $atts;
		$args['id']  						= powernodewt_uniqid('powernode-blockquote-');
		...
		$args['wrap_atts']['class'][] 		= 'powernode-blockquote';
		$args['wrap_atts']['class'][] 		= 'pnwt-blockquote quote';
		...
		if( !empty( $content ) ) {
			$content_attr = array();
			$content_attr['class'] = array( 'quote-content' );
			...
			$css_output .= powernodewt_output_css( array( '#'. $args['id'] . ' .quote-content' => $content_css_ar ) );
		}
...
add_shortcode( 'powernode_blockquote', 'powernodewt_blockquote' );