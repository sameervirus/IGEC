...
$sub_atts = array();
$sub_atts['class'] = array( 'entry-post', 'portfolio-post', 'hover-overlay' );
...
if( $post_style == 'fancy' ) {
	$sub_atts['class'][] = 'portfolio-2-post';
} else if( $post_style == 'box' ) {
	$sub_atts['class'][] = 'portfolio-1-post';
	$content_args['class'][] = 'portfolio-post-txt';
}
?>
<div id="img-4-1" class="gallery-image">
	<div <?php echo powernodewt_portfolio_loop_post_atts(); ?>>
		<?php do_action( 'powernode_before_portfolio_loop_post' ); ?>
		...
		<?php echo get_template_part( 'template-parts/portfolio-loop/thumbnail'  ); ?>
		<div class="item-overlay"></div>
			
		<div <?php echo powernodewt_stringify_atts( $content_args ); ?>>
			<div class="project-meta-txt">
				<?php echo get_template_part( 'template-parts/portfolio-loop/meta' ); ?>
				<?php echo get_template_part( 'template-parts/portfolio-loop/title' ); ?>
				<?php echo get_template_part( 'template-parts/portfolio-loop/content' ); ?>
			</div>
			<?php echo get_template_part( 'template-parts/portfolio-loop/read-more' ); ?>
		</div>
		<?php if ( in_array( $view_type, array( 'list', 'modern' ) ) ) echo '</div>'; ?>
		</div>
		<?php do_action( 'powernode_after_portfolio_loop_post' ); ?>
	</div>
</div>
<?php 
