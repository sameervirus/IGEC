 ...
 * @return void
 */
function powernodewt_breadcrumb_trail( $args = array() ) {

	// Return if breadcrumbs are disabled
	if ( ! powernodewt_page_title_breadcrumbs_display()
		|| is_front_page() ) {
		return;
	}
	...
	if ( !is_object( $breadcrumb ) )
		$breadcrumb = new PowerNodeWT_Breadcrumb_Trail( $args );
 ...
 * @access public
 */
class PowerNodeWT_Breadcrumb_Trail {
	...
	/**
	 * Sets up the breadcrumb trail properties.  Calls the `PowerNodeWT_Breadcrumb_Trail::add_items()` method
	 ...
	 *     @type array     $labels         Text labels. @see PowerNodeWT_Breadcrumb_Trail::set_labels()
	 *     @type array     $post_taxonomy  Taxonomies to use for post types. @see PowerNodeWT_Breadcrumb_Trail::set_post_taxonomy()
	...
	public function get_trail() {
		...
		if ( 0 < $item_count ) {
			...
			// Postion class
			$p_class = '';
			if ( '' != get_theme_mod( 'powernode_breadcrumbs_position' ) ) {
				$p_class = ' position-' . get_theme_mod( 'powernode_breadcrumbs_position' );
			}

			// Wrap the breadcrumb trail.
			$breadcrumb = sprintf(
				'<div class="breadcrumb-nav"><%1$s role="navigation" aria-label="%2$s" class="nav-breadcrumbs'. $p_class .'">%3$s%4$s%5$s</%1$s></div>',
	...
	public function trail() {
		...
		echo apply_filters( 'powernode_breadcrumb_trail', $this->get_trail() );
	}
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:',                               'powernode' ),
			'aria_label'          	=> esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'powernode' ),
			'home'           		=> esc_html__( 'Home', 'powernode' ),
			'error_404'           	=> esc_html__( '404 Not Found', 'powernode' ),
			'archives'            	=> esc_html__( 'Archives',                              'powernode' ),
			// Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
			'search'           		=> esc_html__( 'Search results for', 'powernode' ),
			// Translators: %s is the page number.
			'paged'               	=> esc_html__( 'Page %s',                               'powernode' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s',                       'powernode' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      	=> esc_html__( 'Minute %s',                             'powernode' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        	=> esc_html__( 'Week %s',                               'powernode' ),
	...
	protected function add_network_home_link() {
		...
		$home_style = get_theme_mod( 'powernode_page_title_breadcrumb_home_style', 'text' );
		if ( $home_style == 'icon' ) {
			$icon = '<span class="flaticon-home"></span>';
		} else  {
			$icon = '<span class="breadcrumb-home">'. $this->labels['home'] .'</span>';
		}

		if ( is_multisite() && ! is_main_site() && true === $this->args['network'] )
			$this->items[] = sprintf( '<a href="%s" rel="home">%s</a>', esc_url( network_home_url() ), $icon );
	}
	...
	protected function add_site_home_link() {
		...
		$label   = $network ? get_bloginfo( 'name' ) : $icon;
	...
	protected function add_singular_items() {
		...
		// Post type taxonomy
		$posts_tax 		= get_theme_mod( 'powernode_breadcrumb_posts_taxonomy', 'category' );
		$products_tax 	= get_theme_mod( 'powernode_breadcrumb_products_taxonomy', 'shop' );
		$portfolio_tax 	= get_theme_mod( 'powernode_breadcrumb_portfolio_taxonomy', 'powernode_portfolio_category' );
		...
		// If the post type is 'powernode_portfolio'.
		else if ( class_exists( 'PowerNode_Portfolio' )
			&& 'powernode_portfolio' === $post->post_type
			&& 'none' != $portfolio_tax ) {
		...
		if (get_theme_mod( 'powernode_page_title_breadcrumbs_item_title_display', true ) == true
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'powernode' ) ) );
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'powernode' ) ) );
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'powernode' ) ) );
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'powernode' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'powernode' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'powernode' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'powernode' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'powernode' ) ) );
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'powernode' ) ) ) );

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'powernode' ) ) ) );

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'powernode' ) ) ) );
